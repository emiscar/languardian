        ...
        if formset.is_valid():
            ...
            if res == True:
                ...
                if "Authentication" in res:
                    messages.warning(
                        self.request, "Autenticación fallida. Verifique usuario y contraseña")
                else:
                    if "10060" in res:
                        messages.warning(
                            self.request, "Timeout. Dispositivo sin respuesta")
                    else:
                        if "11001" in res:
                            messages.warning(
                                self.request, "Ingrese una IP válida")
                        else:
                            if "Unable to connect to port" in res:
                                messages.warning(
                                    self.request, "Ingrese un puerto válido")
        return self.render_to_response({'miform': formset})
...
def con_ssh(ip, puerto, usuario, clave, id):
    ...
    c = Connection(host=usuario + "@" + ip,
                   connect_kwargs={"password": clave}, port=puerto)
    try:
        c.run('show run')
...
def configuracion(request):
    ...
    for i in range(int(c_dispo)):
        if i == 0:
            ...
            nist_d.append(nist_demas)
            buep_d.append(buep_demas)
            # LLAMO AL METODO PARA GENERAR PDF
            #reporgen (i, reco_demas)
            generar_pdf(nist_demas, i)
    return render(request, "LanGuardianCont/configuracion.html", {'cont_demas': cont_demas, 'reco_demas': reco_d, 'cant': int(c_dispo), 'cont_uno': cont_uno, 'reco_uno': reco_uno, 'nist_uno': nist_uno, 'buep_uno': buep_uno, 'nist_demas': nist_d, 'buep_demas': buep_d})
...
def generar_pdf(recos, i):
    template = get_template('LanGuardianCont/report.html')
    context = {'nist': recos}
    html = template.render(context)
    response = HttpResponse(content_type='application/pdf')
    response['Content-Disposition'] = 'attachment; filename="report{i}.pdf"'
...
def report_pdf(request):
    ...
    #context = {}
    #html = template.render(context)
    f = open("config0.pdf", "r")
    response = HttpResponse(f, content_type='application/pdf')
    response['Content-Disposition'] = 'attachment; filename=Reporte_0.pdf'
    f.close()
    # create a pdf
